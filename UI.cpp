...
void UI::Render()
{
    ...
    ::RegisterClassEx(&wc);
    const HWND hwnd = ::CreateWindowEx(WS_EX_TOPMOST,wc.lpszClassName, _T("ImGui Standalone"), WS_OVERLAPPEDWINDOW, 100, 100, 50, 50, NULL, NULL, wc.hInstance, NULL);
    ...
    while (!bDone)
    {
        ...
        if (GetAsyncKeyState(VK_F3) & 1) bDone = true;

        if (GetAsyncKeyState(VK_F2) & 1) Drawing::bShowMenu = !Drawing::bShowMenu;
